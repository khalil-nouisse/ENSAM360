...
const seedDatabase = async () => {
    ...
    try {
        ...
        for (const loc of locations) {
            ...
            await session.run(
                `MERGE (n:Location:${loc.label} {id: $id})
                 SET n.name = $name,
        ...
        await session.run(`
            MATCH (b:Building {id: 'AEEE'})
            MATCH (r:Room {id: 'a3einside'})
            MERGE (b)-[:CONTAINS]->(r)
        `);
        ...
        await session.run(`
            MATCH (a:Location {id: 'entree_ecole'})
            MATCH (b:Location {id: 'administration'})
            MERGE (a)-[r1:PATH_TO {distance: 15}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 15}]->(a)
        `);

        await session.run(`
            MATCH (a:Location {id: 'administration'})
            MATCH (b:Location {id: 'Admnetud'})
            MERGE (a)-[r1:PATH_TO {distance: 10}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 10}]->(a)
        `);

        await session.run(`
            MATCH (a:Location {id: 'Admnetud'})
            MATCH (b:Location {id: 'Administration_etud'})
            MERGE (a)-[r1:PATH_TO {distance: 12}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 12}]->(a)
        `);

        await session.run(`
            MATCH (a:Location {id: 'Administration_etud'})
            MATCH (b:Location {id: 'couloirForum'})
            MERGE (a)-[r1:PATH_TO {distance: 8}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 8}]->(a)
        `);
        ...
        await session.run(`
            ...
            MERGE (a)-[r1:PATH_TO {distance: 20}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 20}]->(a)
        `);

        await session.run(`
            MATCH (a:Location {id: 'Bibliotheque_et_centre_de_langue'})
            MATCH (b:Location {id: 'escalier1'})
            MERGE (a)-[r1:PATH_TO {distance: 5}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 5}]->(a)
        `);
        ...
        await session.run(`
            ...
            MERGE (a)-[r1:PATH_TO {distance: 3}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 3}]->(a)
        `);
        ...
        await session.run(`
            ...
            MERGE (a)-[r1:PATH_TO {distance: 18}]->(b)
            MERGE (b)-[r2:PATH_TO {distance: 18}]->(a)
        `);
